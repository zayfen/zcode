...
mod tests {
    ...
    fn test_mcp_flag_three_servers() {
        let args = Args::try_parse_from(["zcode", "--mcp", "s1", "--mcp", "s2", "--mcp", "s3"]);
        assert!(args.is_ok());
        let args = args.unwrap();
    ...
    fn test_multiple_flags_with_run() {
        let args =
            Args::try_parse_from(["zcode", "-v", "-m", "gpt-4", "-M", "server1", "run", "task"]);
        assert!(args.is_ok());
        let args = args.unwrap();
        assert!(args.verbose);
        assert_eq!(args.model, Some("gpt-4".to_string()));
    ...
    fn test_task_run_by_description() {
        ...
        if let Some(Command::Task {
            action: TaskAction::Run { task_or_id, .. },
        }) = args.command
        {
    ...
    fn test_task_run_all_both_flags() {
        let args =
            Args::try_parse_from(["zcode", "task", "run-all", "-j", "3", "-n", "100"]).unwrap();
        if let Some(Command::Task {
            action:
                TaskAction::RunAll {
                    concurrency,
    ...
    fn test_task_run_all_with_model_flag() {
        let args = Args::try_parse_from(["zcode", "-m", "claude-3", "task", "run-all", "-j", "2"])
            .unwrap();
        assert_eq!(args.model, Some("claude-3".to_string()));
        if let Some(Command::Task {
            action: TaskAction::RunAll { concurrency, .. },
        }) = args.command
        {
